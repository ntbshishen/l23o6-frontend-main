...
import { Right, Ticket } from "@element-plus/icons-vue";
...
const submitOrderForm = (formEl: FormInstance | undefined) => {
  if (!formEl) return
  var m:Number
    for(var start=0;start<props.ticket_info?.length;start++){
       if(props.ticket_info[start].type===orderForm.seat_type){
              m=props.ticket_info[start].price
              break
       }
  ...
  formEl.validate((valid) => {
    if (!valid) return
    console.log('submit!')
    //var ticket_info=props.ticket_info
